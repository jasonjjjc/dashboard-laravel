<x-layout>
    <main class="max-w-2xl mt-6 mx-auto flex flex-col md:flex-row">
        ...
        <form method="POST" action="/admin/employees/{{ $employee->slug }}"
            ...
            <!-- Name Input -->
                <x-form-input type="text" id="name" name="name" placeholder="" :value="$employee->name" labeltext="Name" required="false" />
                
            <!-- Company Dropdown-->
            <label for="company_id" class="font-bold text-xs uppercase ">Company</label>
            ...
            <!-- Email Input -->
                <x-form-input type="email" id="email" name="email" placeholder="" :value="$employee->email" labelText="Email" />


            <!-- Job Title Input -->
                <x-form-input type="text" id="job_title" name="job_title" placeholder="" :value="$employee->job_title" labelText="Job Title" />
        

            <!-- Phone Input -->
            <x-form-input type="tel" id="phone" name="phone" placeholder="" :value="$employee->phone" labelText="Phone" />


            <!-- Address Input -->
            <x-form-input type="text" id="address" name="address" placeholder="" :value="$employee->address" labelText="Address" />
            ...
            <div class="flex flex-col">
                ...
                <textarea name="description" rows="6" class="border rounded-md px-4 py-2 mb-2 focus:outline-blue-500">{{ $employee->description }}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm">{{ $message }}</p>
                @enderror
            </div>
        ...
        <form method="POST" action="/admin/employees/{{ $employee->slug }}"
            ...
            <!-- Name Input -->
            <x-form-input type="text" id="name" name="name" placeholder="" :value="$employee->name" labeltext="Name" />
