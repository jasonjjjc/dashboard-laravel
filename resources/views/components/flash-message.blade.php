<div x-data="{ isOpen: true }" x-init="setTimeout(() => isOpen = false, 10000)" class="fixed bottom-0 right-0 p-4" x-show="isOpen">
    <div class="flex items-center gap-8 py-2 pl-4 {{ isset($success) && $success == 'error' ? "bg-red-100 border border-red-400 text-red-700 rounded" : "bg-blue-100 border border-blue-400 text-blue-700 rounded" }}">
        <h1>{{ $message }}</h1>
